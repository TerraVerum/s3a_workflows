...
class DataGenerator(Sequence):
    ...
    def __init__(
        ...
        """
        np.random.seed(22)
        self.ids = owned_images
        self.images_dir = images_dir
        self.masks_dir = label_masks_dir
        ...
        self.output_classes = self._create_output_class_mapping(number_label_map)
    ...
    def _create_output_class_mapping(output_classes: pd.Series):
        ...
        """
        classes, numeric_labels = np.unique(output_classes, return_inverse=True)
    ...
    def num_output_classes(self):
        # In case multiple labels alias to each other
        return len(self.output_classes['numeric_class'].unique())
    ...
    def __getitem__(self, index):
        ...
        ids_temp = [self.ids[i] for i in indexes]
    ...
    def __data_generation(self, ids):
        ...
        :param ids: The file ids of the specific batch of data.
        """
        images = np.empty((len(ids), *self.image_shape, 3), dtype=np.uint8)
        masks = np.empty((len(ids), *self.image_shape, self.num_output_classes), dtype=np.uint8)
        for index, img_file in enumerate(ids):
            directory_data_type = img_file.parent.name
            img = gutils.cvImread_rgb(self.images_dir / directory_data_type / img_file.name, cv.IMREAD_UNCHANGED)
            mask = gutils.cvImread_rgb(str(self.masks_dir/directory_data_type/img_file.name), cv.IMREAD_UNCHANGED)
